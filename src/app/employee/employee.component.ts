...
export class EmployeeComponent {
  ...
  constructor(private fb: FormBuilder, private http: HttpClient, private renderer: Renderer2, private cdRef: ChangeDetectorRef ) {
    this.employeeForm = this.fb.group({
      ...
      // ❌ OLD IMPLEMENTATION: noticePeriod and probationPeriod were at form level
      // noticePeriod: [''],
      // probationPeriod: [''],
      // ✅ NEW IMPLEMENTATION: Removed from form level because these should be per-employee fields
      // WHY CHANGED: Backend expects noticePeriod and probationPeriod for each individual employee, not at the form level
  ...
  createEmployeeGroup(): FormGroup {
    return this.fb.group({
      ...
      // ❌ OLD IMPLEMENTATION: noticePeriod and probationPeriod were strings with validators
      // noticePeriod: ['', Validators.required],
      // probationPeriod: ['', Validators.required],
      // ✅ NEW IMPLEMENTATION: Changed to boolean values with default false
      // WHY CHANGED: Toggle switches should send boolean values (true/false) to backend, not strings
      noticePeriod: [false], // Default to false, will be updated by toggle events
      probationPeriod: [false], // Default to false, will be updated by toggle events
 
 
    });
  }
 
  // ✅ NEW IMPLEMENTATION: Added toggle event handlers to capture toggle switch states
  // WHY ADDED: These methods will update the form values when toggle switches are clicked
  onNoticePeriodToggle(index: number, event: any): void {
    const employeeGroup = this.employees.at(index) as FormGroup;
    employeeGroup.patchValue({ noticePeriod: event.checked });
    console.log(`Employee ${index + 1} notice period: ${event.checked}`); // Debug log
  }
 
  onProbationPeriodToggle(index: number, event: any): void {
    const employeeGroup = this.employees.at(index) as FormGroup;
    employeeGroup.patchValue({ probationPeriod: event.checked });
    console.log(`Employee ${index + 1} probation period: ${event.checked}`); // Debug log
  }
  ...
  // ✅ NEW IMPLEMENTATION: Added debug logging to see what data is being sent
  // WHY ADDED: To verify that noticePeriod and probationPeriod values are correct before sending
